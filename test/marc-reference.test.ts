...
test("parses canonical mARC references", () => {
  ...
  assert.deepEqual(
    parseMarcReference("marc://$thread-abc/#msg_123/!decision-notes.md"),
    {
      type: "artifact",
      threadId: "thread-abc",
      messageId: "msg_123",
...
test("rejects malformed mARC references", () => {
  ...
  assert.equal(
    parseMarcReference("marc://$thread-abc/!decision-notes.md"),
    undefined
  );
  assert.equal(
    parseMarcReference("marc://#msg_123/artifacts/decision-notes.md"),
    undefined
  );
