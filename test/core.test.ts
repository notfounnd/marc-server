...
test("updates workspace recommendations idempotently", async () => {
  ...
  assert.deepEqual(second.alreadyCurrent.sort(), [".agents/skills/marc-ops/SKILL.md", "INSTRUCTIONS.md", "RULES.md"]);
  ...
  assert.match(rules, /## Operational Custom Rules/);
  assert.match(rules, /`Trigger`/);
  assert.match(rules, /`Do instead`/);
  assert.match(rules, /`Evidence`/);
  assert.match(rules, /`Severity`/);
  assert.doesNotMatch(rules, /`register_agent`/);
  assert.doesNotMatch(rules, /Registered Agents \(Marckers\)/);

  const skill = await fs.readFile(path.join(workspace, ".agents", "skills", "marc-ops", "SKILL.md"), "utf8");
  assert.match(skill, /^---\nname: marc-ops\n/m);
  assert.match(skill, /^description: \|\n  Operate inside a mARC-enabled repository\. Activates EVERY session in this workspace\./m);
  assert.match(skill, /^when_to_use: \|/m);
  assert.match(skill, /^# mARC Ops$/m);
  assert.match(skill, /workspace_bootstrap/);
  assert.match(skill, /RULES\.md/);
  assert.match(skill, /Custom Rules/);
  assert.match(skill, /## Always Active/);
  assert.match(skill, /## Context of Usage/);
  assert.match(skill, /Always when starting a session in a mARC workspace/);
  assert.match(skill, /Always before reading or continuing a thread/);
  assert.match(skill, /## Preflight Checklist/);
  assert.match(skill, /## Audit Discipline/);
  assert.match(skill, /workspace_audit/);
  assert.match(skill, /Attach artifacts before referencing them/);
});
...
test("audits free-form custom rules as operational guidance warnings", async () => {
  ...
  assert.equal(result.findings[0].location, ".marc/RULES.md#custom-rules-agent-tooling");
});
...
test("audits workspace content for rules, artifacts, references, agents, and preflight", async () => {
  ...
  await fs.writeFile(
    ...
    ["# broken-agent", "", "ID: `other-agent`", "Role: reviewer", "Model: human", "Description: bad", ""].join("\n"),
  );
  await fs.appendFile(
    path.join(workspace, ".marc", "RULES.md"),
    [
...
test("resolves references to closed threads", async () => {
  ...
  await fs.writeFile(path.join(workspace, ".marc", "threads", target.id, "SUMMARY.md"), "# Summary\n");
  ...
  assert.equal(result.ok, true);
  assert.ok(!result.findings.some((finding) => finding.code === "reference_thread_missing"));
});
...
test("reports missing agent metadata as warnings", async () => {
  ...
  await fs.writeFile(
    ...
    ["# incomplete-agent", "", "ID: `incomplete-agent`", "Role: reviewer", "Model: human", ""].join("\n"),
  );
  ...
  const metadataFinding = result.findings.find((finding) => finding.code === "agent_metadata_missing");
  assert.equal(result.ok, true);
...
test("deduplicates repeated findings in all scope", async () => {
  ...
  const invalidReferences = result.findings.filter((finding) => finding.code === "reference_invalid");
