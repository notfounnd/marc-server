...
test("suggests agents, workspace threads, current messages, and current artifacts", () => {
  ...
  assert(agentRequest);
  assert.deepEqual(
    buildAutocompleteOptions(agentRequest, { agents, threads, currentMessages }).map((option) => option.value),
    ["marc://@codex-dev"],
  );

  const threadRequest = detectAutocompleteRequest("$referencias", 12);
  assert(threadRequest);
  const threadOptions = buildAutocompleteOptions(threadRequest, { agents, threads, currentMessages });
  assert.deepEqual(threadOptions.map((option) => option.value), [
  ...
  assert.deepEqual(
    threadOptions.map((option) => ({ detail: option.detail, closed: Boolean(option.closed) })),
    [
  ...
  const messageOptions = buildAutocompleteOptions(messageRequest, { agents, threads, currentMessages });
  assert.deepEqual(messageOptions.map((option) => option.value), [
...
test("suggests cross-thread messages and artifacts only from an explicit thread reference", () => {
  ...
  assert(request);
  assert.equal(getAutocompleteRemoteThreadId(request), "oportunidade-referencias-internas-e-mencoes-direcionadas-3c4261b4");
...
test("replaces only the active token with the selected canonical reference", () => {
  ...
  assert(request);
  assert.deepEqual(applyAutocompleteOption("Ask @cod to review", request, "marc://@codex-dev"), {
    cursor: 21,
