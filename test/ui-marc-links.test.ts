...
import { linkifyMarcReferences, marcReferenceLabel, transformMarkdownUrl } from "../src/ui/marc-links.js";

test("renders canonical short labels for mARC references", () => {
  assert.equal(marcReferenceLabel("marc://@codex-dev"), "@codex-dev");
  assert.equal(marcReferenceLabel("marc://#msg_3937368093ec46fead"), "#msg_3937368093ec46fead");
  assert.equal(
    marcReferenceLabel("marc://$oportunidade-referencias-internas-e-mencoes-direcionadas-3c4261b4"),
    "$oportunidade-referencias-internas-e-mencoes-direcionadas-3c4261b4",
  );
  assert.equal(
    marcReferenceLabel("marc://#msg_d8d44b0250e842ffa2/!complemento-artifacts-referencias.md"),
...
test("does not linkify mARC references inside inline code, fenced code, or markdown destinations", () => {
  assert.equal(linkifyMarcReferences("Use `marc://@codex-dev` literally."), "Use `marc://@codex-dev` literally.");
  assert.equal(
...
test("preserves mARC URLs while keeping the markdown default transform for other URLs", () => {
  ...
  assert.equal(transformMarkdownUrl("https://example.com"), "https://example.com");
});
