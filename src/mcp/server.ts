...
function daemonConnection(options: McpOptions): { daemonUrl: string; token: string } | undefined {
  const daemonUrl = options.daemonUrl ?? process.env.MARC_DAEMON_URL;
  const token = options.token ?? process.env.MARC_TOKEN;
...
async function renewDaemonLease(
  connection: { daemonUrl: string; token: string },
  clientId: string,
  workspace: WorkspaceInfo,
): Promise<void> {
  await fetch(new URL(`/api/leases/${encodeURIComponent(clientId)}`, connection.daemonUrl), {
    method: "PUT",
    headers: {
      "content-type": "application/json",
...
async function startDaemonLeaseHeartbeat(options: McpOptions): Promise<NodeJS.Timeout | undefined> {
  ...
  const renew = () => {
    void renewDaemonLease(connection, clientId, workspace).catch(() => undefined);
  };
  renew();
  ...
  const cleanup = () => {
    clearInterval(timer);
    void fetch(new URL(`/api/leases/${encodeURIComponent(clientId)}`, connection.daemonUrl), {
      method: "DELETE",
