export type MarcReference =
  ...
  | { type: "artifact"; threadId?: string; messageId: string; artifactFile: string };
...
export function parseMarcReference(value: string): MarcReference | undefined {
  ...
  const localArtifactMatch = new RegExp(`^marc://#(${ID_PATTERN})/!(${FILE_PATTERN})$`).exec(value);
  ...
  if (localMessageMatch) {
    return { type: "message", messageId: decodeURIComponent(localMessageMatch[1]) };
  }

  const remoteArtifactMatch = new RegExp(`^marc://\\$(${ID_PATTERN})/#(${ID_PATTERN})/!(${FILE_PATTERN})$`).exec(value);
  ...
  const remoteMessageMatch = new RegExp(`^marc://\\$(${ID_PATTERN})/#(${ID_PATTERN})$`).exec(value);
...
export function messageArtifactReference(messageId: string, artifact: string, threadId?: string): string {
